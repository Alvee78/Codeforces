...
void column(ll idx,vector<ll>& a,vector<ll>& b,vector<ll>& v, ll m){
    if(idx == m)return;
    v.push_back(a[idx]+b[idx]);
    column(idx+1,a,b,v,m);
}

void row(ll idx,vector<vector<ll>>& a,vector<vector<ll>>& b,ll n, ll m){
    if(idx == n)return;
    vector<ll> v;
    column(0,a[idx],b[idx],v,m);
    ans.push_back(v);
    row(idx+1,a,b,n,m);
}
