...
ll dp[30];
ll solve(ll idx,ll n,ll x,vector<pair<ll,ll>>& v,ll sum,ll w){
    if(w>x)return -1;
    if(idx == n)return sum;
    ll take;
    if(dp[idx] != -1)take = dp[idx];
    take = dp[idx] = solve(idx+1,n,x,v,v[idx].second+sum,w+v[idx].first);
    ll nottake = solve(idx+1,n,x,v,sum,w);
